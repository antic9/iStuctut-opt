...
def evaluate(model, batches):
    ...
    with torch.no_grad():
        for inputs, targets in batches:
            losses, _ = model.autoenc(inputs, targets)
...
def main(args):
    ...
    torch.set_printoptions(edgeitems=100000000000000)
    if not os.path.exists(args.save_dir):
        os.makedirs(args.save_dir)
    log_file = os.path.join(args.save_dir, 'log.txt')
    mu_log_file = os.path.join(args.save_dir, 'mu_log.txt')
    sigma_log_file = os.path.join(args.save_dir, 'sigma_log.txt')
    logging(str(args), log_file)

    # Prepare data
    # train_sents = load_sent(args.train)
    data_sents = load_sent(args.train)
    random.shuffle(data_sents)
    divindex = int(len(data_sents) * 0.8)

    train_sents = data_sents[0:divindex-1]
    valid_sents = data_sents[divindex:len(data_sents)]
    ...
    # print(x.data.nelement() for x in model.parameters())
    print("hello")
    save2=args.save_dir+'/train.txt'
    shutil.copyfile(args.train,save2)
    ...
    for epoch in range(args.epochs):
        ...
        for i, idx in enumerate(indices):
            ...
            print(inputs)
            losses, musigma = model.autoenc(inputs, targets, is_train=True)
        ...
        if not best_val_loss or valid_meters['loss'].avg < best_val_loss:
            ...
            with open(sigma_log_file,'w+') as f:
                f.write(str(musigma['sigma'].data))
            with open(mu_log_file,'w+') as f:
                f.write(str(musigma['mu'].data))
        logging(log_output, log_file)
